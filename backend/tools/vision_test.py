...
E_IMG_PATH = os.path.join(BASE_DIR, "pictures", "E.png")


class VisionTester:
    def __init__(self, camera_index: int = 0, target_distance_cm: int = 60):
        # 延迟导入 mediapipe，避免模块级导入时的 tensorflow 依赖问题
        try:
            import mediapipe as mp
        except ImportError as e:
            error_msg = str(e)
            if 'tensorflow' in error_msg.lower() or 'doc_controls' in error_msg:
                raise ImportError(
                    f"无法导入 mediapipe，TensorFlow 版本不兼容。\n"
                    f"请运行: pip install tensorflow==2.13.0\n"
                    f"或参考 FIX_MEDIAPIPE.md 文件。\n"
                    f"原始错误: {error_msg}"
                ) from e
            else:
                raise ImportError(f"无法导入 mediapipe，请确保已安装: {error_msg}") from e
        ...
        # 加载E字图片
        self.e_image = self._load_e_image(E_IMG_PATH)
    ...
    def _init_test_parameters(self):
        ...
        self.total_count = 0
        self.distance = self.target_distance_cm  # 初始距离(厘米)
        ...
        self.round_directions = []  # 当前一轮收集到的手势方向
        self.STABLE_WINDOW = 5
            ...
            while self.testing and self.cap.isOpened():
                ...
                if hand_results.multi_hand_landmarks:
                    ...
                    if hand_direction is not None:
                        self.round_directions.append(hand_direction)
                        # 控制列表长度
                        if len(self.round_directions) > self.STABLE_WINDOW:
                            self.round_directions.pop(0)
                ...
                if time.time() - self.last_change_time >= self.display_duration:
                    stable_dir = self._majority_direction()
                    if stable_dir is not None:
                        self._process_direction_match(stable_dir)
                    else:
                        # 无有效手势按错误处理，促使字号放大
                        self._process_direction_match(-1)
                    self.round_directions.clear()
    ...
    def _calibrate_distance(self):
        ...
        if valid_samples:
            ...
            if abs(avg_distance - self.target_distance_cm) > 10:  # 距离偏差超过10cm时提示
                print(f"检测到您当前距离为{avg_distance:.1f}cm，建议调整至{self.target_distance_cm}cm标准距离")
        cv2.destroyWindow('Calibration')

    def _majority_direction(self):
        """对当前轮的手势方向取众数，提升鲁棒性"""
        if not self.round_directions:
            return None
        vals, counts = np.unique(self.round_directions, return_counts=True)
        max_idx = np.argmax(counts)
        if counts[max_idx] >= self.MIN_STABLE_HITS:
            return int(vals[max_idx])
        return None
